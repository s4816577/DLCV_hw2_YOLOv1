...
def Debug(model, trainset_loader):
    count = 0
    for batch_idx, (datas, labels) in enumerate(trainset_loader):
        print(batch_idx)
        print(datas)
        print(labels)
        if (batch_idx==count):
            datas = transforms.functional.to_pil_image(datas[0])
            #datas = datas.resize((512,512), Image.ANTIALIAS)
            datas.save('hi.jpg')
            with open('hi.txt', 'w') as f:
                for i in range(len(labels[0])):
                    print("{} {} {} {} {} {} {} {} {} {}".format(labels[0][i][0], labels[0][i][1], -1, -1, labels[0][i][2], labels[0][i][3], -1, -1, DOTA_CLASSES[labels[0][i][4]], -1), file=f)
...
def NMS_and_output(datas):
    ...
    current_validate_img += 1
    output_filename = 'output/{0:04}.txt'.format(current_validate_img)
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(testset_loader):
            ...
            if iteration % log_interval == 0:
                print('Valisate Epoch: [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                    batch_idx * len(data), len(testset_loader.dataset),
                    100. * batch_idx / len(testset_loader), loss.item()))
            iteration += 1
...
def main():
    ...
    #windows
    trainset = localLoader.AERIAL('hw2_train_val/train15000', transform=transforms.ToTensor())
    trainset_loader = DataLoader(trainset, batch_size=32, shuffle=True, num_workers=1, collate_fn=collate_fn)
    
    testset = localLoader.AERIAL('hw2_train_val/val1500', transform=transforms.ToTensor())
    testset_loader = DataLoader(testset, batch_size=32, shuffle=False, num_workers=1, collate_fn=collate_fn)
